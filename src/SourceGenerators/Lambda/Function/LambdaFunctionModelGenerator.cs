...
public static class LambdaFunctionModelGenerator {
    public static RequestHandlerModel GenerateRequestModel(GeneratorSyntaxContext context,
        ...
        return new RequestHandlerModel(
            new RequestHandlerNameModel(GetFunctionName(context, methodDeclaration, methodName), "Invoke"),
            controllerType,
            methodName,
            TypeDefinition.Get(controllerType.Namespace + ".Generated",
                "InvokeFilter" + controllerType.Name + "_" + methodName),
            GetParameters(context, methodDeclaration, cancellation), response, filters);
    }

    private static string GetFunctionName(GeneratorSyntaxContext generatorSyntaxContext,
        MethodDeclarationSyntax methodDeclaration, string methodName) {
        var attribute =
            methodDeclaration.GetAttribute(
                KnownTypes.Requests.HardenedFunctionAttribute.Name.Replace("Attribute", ""))!;
        var argument = attribute.ArgumentList?.Arguments.FirstOrDefault();
    ...
    private static RequestParameterInformation? GetParameterInfoFromAttributes(
        GeneratorSyntaxContext generatorSyntaxContext,
        ParameterSyntax parameter) {
        foreach (var attributeList in parameter.AttributeLists) {
            foreach (var attribute in attributeList.Attributes) {
